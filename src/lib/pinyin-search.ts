...
export function tone(s: string): number {
...
function _hasToneNum(tok: string): boolean {
  return /[a-züāáǎàēéěèīíǐìōóǒòūúǔùǖǘǚǜ][1-4]/i.test(tok)
}

function _applyTone(syl: string, num: string): string {
  ...
  for (const v of [...s].reverse()) {
    if (_TONED[v as keyof typeof _TONED])
      return s.slice(0, s.lastIndexOf(v)) + _TONED[v as keyof typeof _TONED][n] + s.slice(s.lastIndexOf(v) + 1)
  }
  return s
}

function _numToTone(str: string): string {
  const s = str.replace(/v([1-4])/g, "ü$1").replace(/v(?=[^1-4]|$)/g, "ü")
  return s.replace(
...
function _isSylBoundary(after: string | undefined): boolean {
...
export function matchPinyinTokens(pyDB: string, queryTokens: PinyinToken[]): boolean {
  ...
  for (const tok of queryTokens) {
    ...
    while (true) {
      ...
      if (p === -1) break
      if (p >= pos && _isSylBoundary(src[p + q.length])) {
        pos = p + q.length
        found = true
        break
      }
...
export function getPinyinRegex(query: string): string {
  const clean = stripTones(query.toLowerCase())
...
export function isIndonesianQuery(raw: string): boolean {
